...
contract VRFCoordinatorV2Mock is VRFCoordinatorV2Interface {
    ...
    function getSubscriptionIds(address owner)
        public
        view
        returns (uint64[] memory)
    {
        uint64 [] memory tempSubIds = new uint64[](100); 
        uint64 count;
        for (uint64 i = 1; i <= s_currentSubId; i++) {
            if (s_subscriptions[i].owner == owner) {
                tempSubIds[count] = i;
                count++;
            }
        }
        uint64 [] memory subIds = new uint64[](count);
        for (uint64 i = 0; i < count; i++) {
            subIds[i] = tempSubIds[i];
        }
        return subIds;
    }
}
